...
enum
{
	...
	GUI_ID_REFRESH_MODLIST,
	GUI_ID_INSTALL_MOD,
	GUI_ID_UNINSTALL_MOD,
	GUI_ID_DELETE_WORLD_BUTTON,
	GUI_ID_CREATE_WORLD_BUTTON,
	GUI_ID_CONFIGURE_WORLD_BUTTON,
	GUI_ID_WORLD_LISTBOX,
	GUI_ID_MODLIST,
	GUI_ID_MODMANAGER_GAMELIST,
	GUI_ID_TAB_CONTROL,
	GUI_ID_MODMANAGER_STATUS,
	GUI_ID_MODLIST_LOCAL,
	GUI_ID_MODMANAGER_INFO,
	GUI_ID_SEARCHMOD_INPUT,
};
	...
	m_is_regenerating(false)
{
	...
	m_data->modmanager_status = wgettext(" Click 'Refresh' to show a list of available mods\n (network connection required)");
}
...
void GUIMainMenu::regenerateGui(v2u32 screensize)
{
	...
	*/
	...
	else if(m_data->selected_tab == TAB_MODS)
	{
		s32 border_left_x = 50;
		s32 border_right_x = 0;
		s32 border_y = 50;
		s32 separator_x = 10;

		s32 left_x = m_size_client.X / 2 - border_left_x - separator_x / 2;
		s32 right_x = m_size_client.X / 2 - border_left_x - separator_x / 2 - border_right_x;
		...
		// Gamelist
		{
			core::rect<s32> rect(0, 0, left_x, 80);
			rect += m_topleft_client + v2s32(border_left_x, border_y);
			gui::IGUIListBox *gamelist = Environment->addListBox(rect, this,
					GUI_ID_MODMANAGER_GAMELIST);
			gamelist->setDrawBackground(true);
			std::vector<SubgameSpec> games = getAvailableGames();
			for (u32 i=0; i<games.size(); i++){
				std::wostringstream os(std::ios::binary);
				os<<narrow_to_wide(games[i].name).c_str();
				os<<L" [";
				os<<narrow_to_wide(games[i].id).c_str();
				os<<L"]";
				gamelist->addItem(os.str().c_str());
			}
		...
		// LocalModsList
		{
			core::rect<s32> rect(0, 0, left_x, 110);
			rect += m_topleft_client + v2s32(border_left_x, border_y + 90);
			gui::IGUIListBox *localmodslist = Environment->addListBox(rect, this,
					GUI_ID_MODLIST_LOCAL);
			localmodslist->setDrawBackground(true);
		...
		// Modlist
		{
			core::rect<s32> rect(0, 0, right_x, 110);
			rect += m_topleft_client + v2s32(border_left_x + left_x + separator_x, border_y);
			gui::IGUIListBox *modlist = Environment->addListBox(rect, this,
					GUI_ID_MODLIST);
			modlist->setDrawBackground(true);
		}
		// Search Field
		{
			core::rect<s32> rect(0, 0, right_x, 30);
			rect += m_topleft_client + v2s32(border_left_x + left_x + separator_x, border_y + 115);
			gui::IGUIElement *e = 
			Environment->addEditBox(m_data->address.c_str(), rect, true,
					this, GUI_ID_SEARCHMOD_INPUT);
			if(m_data->name != L"" && m_data->address == L"")
				Environment->setFocus(e);
		}
		// Refresh modlist button
		{
			core::rect<s32> rect(0, 0, right_x/2, 30);
			rect += m_topleft_client + v2s32(border_left_x + left_x + separator_x, border_y + 150);
			Environment->addButton(rect, this,
					GUI_ID_REFRESH_MODLIST, wgettext("Refresh"));
		}
		// Install mod button
		{
			core::rect<s32> rect(0, 0, right_x/2, 30);
			rect += m_topleft_client + v2s32(border_left_x + left_x + separator_x + right_x / 2, border_y + 150);
			Environment->addButton(rect, this,
					GUI_ID_INSTALL_MOD, wgettext("Install"));
		}
		// Uninstall mod button
		{
			core::rect<s32> rect(0, 0, right_x/2, 30);
			rect += m_topleft_client + v2s32(border_left_x + left_x + separator_x, border_y + 180);
			Environment->addButton(rect, this,
					GUI_ID_UNINSTALL_MOD, wgettext("Uninstall"));
		}
	}
	else if(m_data->selected_tab == TAB_CREDITS)
	{
		...
		}
		{
			...
			"and contributors: tango_, kahrl (kaaaaaahrl?), erlehmann (the hippie), SpeedProg, JacobF (sqlite worlds), teddydestodes, marktraceur, darkrose, Jonathan Neuschäfer (who the hell?), Felix Krausse (broke liquids, IIRC), sfan5, Jeija... and >10 more random people."
...
void GUIMainMenu::drawMenu()
{
	...
	else if(getTab() == TAB_MODS)
	{
		...
		{
			core::rect<s32> rect(0, 0, m_size_server.X, m_size_server.Y/3);
			rect += AbsoluteRect.UpperLeftCorner + m_topleft_server + v2s32(0, m_size_server.Y/2 + m_size_server.Y/12);
			driver->draw2DRectangle(bgcolor, rect, &AbsoluteClippingRect);
			IGUIElement *label = getElementFromId(GUI_ID_MODMANAGER_STATUS);
			if (label != NULL) label->remove(); //remove the old text
			Environment->addStaticText(m_data->modmanager_status,
			rect, false, true, this, GUI_ID_MODMANAGER_STATUS);
		}
		{
			core::rect<s32> rect(0, 0, m_size_server.X, m_size_server.Y/2);
			rect += AbsoluteRect.UpperLeftCorner + m_topleft_server;
			driver->draw2DRectangle(bgcolor, rect, &AbsoluteClippingRect);
		}
	}
...
bool GUIMainMenu::OnEvent(const SEvent& event)
{
	...
	if(event.EventType==EET_GUI_EVENT)
	{
		...
		if(event.GUIEvent.EventType==gui::EGET_LISTBOX_CHANGED && event.GUIEvent.Caller->getID() == GUI_ID_MODMANAGER_GAMELIST)
		{
			((gui::IGUIListBox*)getElementFromId(GUI_ID_MODLIST))->clear();
			refreshLocalModlist();
			IGUIElement *label = getElementFromId(GUI_ID_MODMANAGER_INFO);
			if (label) label->remove();
			return true;
		}
		if(event.GUIEvent.EventType==gui::EGET_EDITBOX_CHANGED && event.GUIEvent.Caller->getID() == GUI_ID_SEARCHMOD_INPUT)
		{
			gui::IGUIEditBox *searchbox = (gui::IGUIEditBox*)getElementFromId(GUI_ID_SEARCHMOD_INPUT);
			gui::IGUIListBox *modlist = (gui::IGUIListBox*)getElementFromId(GUI_ID_MODLIST);
			int count = modlist->getItemCount();
			std::wstring searchpattern(searchbox->getText());
			std::wstring listitem;
			for (int i = 0; i < count; i++)
			{
				listitem = modlist->getListItem(i);
				if (listitem.find(searchpattern) != listitem.npos)
				{
					modlist->setSelected(i);
					return true;
				}
			}
			return true;
		}
		if(event.GUIEvent.EventType==gui::EGET_EDITBOX_ENTER && event.GUIEvent.Caller->getID() == GUI_ID_SEARCHMOD_INPUT)
		{
			((gui::IGUIEditBox*)getElementFromId(GUI_ID_SEARCHMOD_INPUT))
			->setText(narrow_to_wide("").c_str());
			return true;
		}
		if(event.GUIEvent.EventType==gui::EGET_LISTBOX_CHANGED && event.GUIEvent.Caller->getID() == GUI_ID_MODLIST)
		{
			std::string path = getSelectedGamePath();
			if (path == "") return true;
			ModManager *modmanager = new ModManager(path);

			gui::IGUIListBox *modlist = (gui::IGUIListBox*)getElementFromId(GUI_ID_MODLIST);
			int id = modlist->getSelected();
			if (id == -1) 
			{
				IGUIElement *label = getElementFromId(GUI_ID_MODMANAGER_INFO);
				if (label) label->remove();
			...
			ModManagerSpec spec = modmanager->info(wide_to_narrow(modlist->getListItem(id)).c_str());
			if (!createErrorStatus(spec.name))
				createModDescription(spec);
		...
		if(event.GUIEvent.EventType==gui::EGET_BUTTON_CLICKED)
		{
			...
			case GUI_ID_REFRESH_MODLIST: {
				...
				std::string *modnames = modmanager->refresh();

				if (createErrorStatus(modnames[0])) return true;

				int i = 1;
				while (modnames[i] != "")
				{
					modlist->addItem(narrow_to_wide(modnames[i]).c_str());
					i++;
				}
				...
				m_data->modmanager_status = wgettext("Modlist Refreshed");
				return true;
			}
			case GUI_ID_INSTALL_MOD: {
				std::string path = getSelectedGamePath();
				if (path == "") return true;
				ModManager *modmanager = new ModManager(path);
				...
				if (id == -1)
				{
					m_data->modmanager_status = wgettext("Error: No mod selected");
					return true;
				}


				std::string result = modmanager->install(wide_to_narrow(modlist->getListItem(id)).c_str());

				if (!createErrorStatus (result))
				{
					m_data->modmanager_status = wgettext("Success!");
				}
			...
			case GUI_ID_UNINSTALL_MOD: {
				std::string path = getSelectedGamePath();
				if (path == "") return true;
				ModManager *modmanager = new ModManager(path);
				gui::IGUIListBox *modlist = (gui::IGUIListBox*)getElementFromId(GUI_ID_MODLIST_LOCAL);
				int id = modlist->getSelected();
				if (id == -1)
				{
					m_data->modmanager_status = wgettext("Error: No mod selected");
				...
				std::string result = modmanager->uninstall(wide_to_narrow(modlist->getListItem(id)).c_str());
				if (!createErrorStatus (result)) m_data->modmanager_status = wgettext("Successfully uninstalled!");
...
std::string GUIMainMenu::getSelectedGamePath()
{
				...
				it = gameids.begin();
				for (char i=0; i<gamelist_id; i++) 
				{
					it++;
				}
				std::string gameid(*it);

				SubgameSpec spec = findSubgame(gameid);
				return spec.path;
}

void GUIMainMenu::refreshLocalModlist()
{
				gui::IGUIListBox *localmodslist = (gui::IGUIListBox*)getElementFromId(GUI_ID_MODLIST_LOCAL);
				localmodslist->clear();
				std::string gamepath = getSelectedGamePath();
				if (gamepath == "") return;
				std::string modspath = gamepath + DIR_DELIM + "mods";
				std::vector<fs::DirListNode> moddir = fs::GetDirListing(modspath);
				for(u32 j=0; j<moddir.size(); j++)
				{
					if(!moddir[j].dir) continue;
					localmodslist->addItem(narrow_to_wide(moddir[j].name).c_str());
				}
				localmodslist->setSelected(0);	
}

bool GUIMainMenu::createErrorStatus(std::string errorcode) //true: error found; false: no error
{
	if (errorcode == "error.lua")
	{
		m_data->modmanager_status = wgettext("Lua error. See the console output. \n(internet connection? central server online?)");
		return true;
	}
	if (errorcode == "error.oldgame")
	{
		m_data->modmanager_status = wgettext("Error: The game you selected seems to be an old one\n--> Does not contain a modmanager.lua file");
		return true;
	}
	if (errorcode == "error.modlist")
	{
		m_data->modmanager_status = wgettext("Downloading modlist failed. See the console output.");
		return true;
	}
	if (errorcode == "error.install")
	{
		m_data->modmanager_status = wgettext("Mod could not be installed (Download/Unpacking failed).\nSee the console output.");
		return true;
	}
	if (errorcode == "error.uninstall")
	{
		m_data->modmanager_status = wgettext("Mod could not be uninstalled.\nWrite Permission for mod directory?");
		return true;
	}
	if (errorcode == "error.info")
	{
		m_data->modmanager_status = wgettext("Mod info not found.");
		return true;
	}
	return false;
}

void GUIMainMenu::createModDescription(ModManagerSpec spec)
{
	core::rect<s32> rect(0, 0, m_size_server.X, m_size_server.Y/2);
	rect += AbsoluteRect.UpperLeftCorner + m_topleft_server;
	IGUIElement *label = getElementFromId(GUI_ID_MODMANAGER_INFO);
	...
	std::string modmanager_info;
	modmanager_info = "Url:		" + spec.url + "\n"
